...
function reg(){
	...
	if(!empty($_POST)){
		...
		if (empty($errors)){
			...
			$sql="INSERT INTO KeMBkasutajad (username, passwd) VALUES ('$user', SHA1('$pass'))";
...
function logi(){
	...
	if(!empty($_POST)){
		...
		if (empty($errors)){
			...
			$sql="SELECT id, username, role FROM KeMBkasutajad WHERE username = '$user' AND passwd = SHA1('$pass')";
			$result = mysqli_query($L, $sql);
...
function post_it(){
	...
	if(!empty($_POST)){
		...
		if (empty($errors)){
			...
			$sql="INSERT INTO KeMBpostitused (title, content, kasutaja_id, postedat) VALUES ('$title', '$content', $user, NOW() )";
			$result = mysqli_query($L, $sql);
			if ($result){
				// k̵ik ok
...
if (!empty($_GET['id'])) {
	...
	$sql = "SELECT * FROM KeMBpostitused WHERE id=$id ";
		$result = mysqli_query($L, $sql);
	...
	if(!empty($_POST)){
		...
		if (empty($errors)){
			...
			$sql="UPDATE KeMBpostitused SET title='$title', content='$content' WHERE id=$id";
			$result = mysqli_query($L, $sql);
			if ($result){
				// k̵ik ok
	...
	if (empty($_SESSION['user'])) {
		...
		exit(0);
				...
				if (empty($_POST['content'])){
					...
					$sql = "INSERT INTO KeMBkommentaarid (kasutaja_id, postitus_id, content, postedat) VALUES ($uid, $pid, '$cont', NOW() )";
...
function hangi_kommentaarid($pid){
	global $L;
	$kommid=array();
	$sql = "SELECT c.*, k.username as kommenteerija FROM KeMBkommentaarid c, KeMBkasutajad k WHERE postitus_id=$pid AND k.id = c.kasutaja_id";
	$result = mysqli_query($L, $sql);
...
function kuva_post() {
	...
	if (!empty($_GET['id'])) {
		$id = mysqli_real_escape_string($L,$_GET['id']);
		$sql = "SELECT p.*, k.username as postitaja FROM KeMBpostitused p, KeMBkasutajad k WHERE p.id=$id AND k.id=p.kasutaja_id";
		$result = mysqli_query($L, $sql);
		if ($result && mysqli_num_rows($result)>0){
			$post=mysqli_fetch_assoc($result);
			// uus kommentaar?
...
function hangi_postitused(){
	global $L;
	$tulemused=array();
	$sql = "SELECT p.*, k.username as postitaja, (SELECT count(*) FROM KeMBkommentaarid WHERE postitus_id = p.id ) as komme FROM KeMBpostitused p, KeMBkasutajad k WHERE k.id=p.kasutaja_id ORDER BY postedat DESC LIMIT 10";
	$result = mysqli_query($L, $sql);
	while ($r=mysqli_fetch_assoc($result)){
		$tulemused[]=$r;
	}
...
function kuva_postitused(){
	...
	if (!empty($_GET['user'])) {
		...
		$sql = "SELECT p.*, k.username as postitaja, (SELECT count(*) FROM KeMBkommentaarid WHERE postitus_id = p.id ) as komme FROM KeMBpostitused p, KeMBkasutajad k WHERE k.id=p.kasutaja_id AND k.username='$user' ";
		$result = mysqli_query($L, $sql);
		while ($r=mysqli_fetch_assoc($result)){
			$postitused[]=$r;
		}
