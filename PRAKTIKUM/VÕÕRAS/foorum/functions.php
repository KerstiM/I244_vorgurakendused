...
function kuva_write(){
	if(!empty($_SESSION["user"])) {
		...
		$_SESSION["teade"] = "Foorumipostituse lisamiseks logi sisse!";
		header("Location: ?mode=avaleht");
		exit(0);
	}
}
...
function kuva_reply(){
	if(!empty($_SESSION["user"])) {
		...
		$_SESSION["teade"] = "Foorumipostitusele vastamiseks logi sisse!";
