	...
	switch($mode){
		...
		case "vote":
			if(!empty($_SESSION["voted_for"])){
				header("Location: http://enos.itcollege.ee/~mputsepp/V%C3%B5rgurakendused/Autentimine/kontrolleriga/kontroller.php?mode=tulemus");
				exit(0);
			} else {
				include("vote.html");
			}
		...
		case "logout":
			...
			session_destroy();
			header("Location: http://enos.itcollege.ee/~mputsepp/V%C3%B5rgurakendused/Autentimine/kontrolleriga/kontroller.php?mode=pealeht");
			exit(0);
		default:
			include("pealeht.html");
			break;
